...
export function lanceManette(manette, input) {
  ...
  manette.play('manette_lance')
    const kunai = manette.scene.add.existing(new ManetteClass(manette.scene, manette.flipX ? manette.x - 80 : manette.x + 80, manette.y - 60, "huzounet",  `${(Math.random() + 1).toString(36).substring(7)}`)
